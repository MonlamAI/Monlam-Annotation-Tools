"""
Patched data_export/models.py for Doccano.

Fixes the export to include the actual audio URL (filename field) instead of
upload_name for Speech2Text projects.
"""
...
class ExportedExample(Example):
    ...
    def to_dict(self, is_text_project=True) -> Dict[str, Any]:
        """
        Convert example to dictionary for export.
        
        PATCHED: For non-text projects (Speech2Text, Image, etc.), use the actual
        filename field which contains the audio/image URL, not upload_name.
        """
        if is_text_project:
            data_value = self.text
        else:
            # Get the actual filename (audio URL for STT)
            ...
            if filename_str.startswith('http://') or filename_str.startswith('https://'):
                data_value = filename_str
            elif self.filename:
                # For local files, use the file URL
                data_value = self.filename.url if hasattr(self.filename, 'url') else str(self.filename)
            else:
                # Fallback to upload_name
                data_value = self.upload_name
        
        return {"id": self.id, DATA: data_value, **self.meta}
