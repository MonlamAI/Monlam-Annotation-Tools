...
# Speech to Text - JSONL format (Monlam patch for importing with audio URLs)
Options.register(
    Option(
        display_name=JSONL.name,
        task_id=ProjectType.SPEECH2TEXT,
        file_format=JSONL,
        arg=ArgColumn,
        file=SPEECH_TO_TEXT_DIR / "example.jsonl",
    )
)

# Image Classification - JSONL format (Monlam patch for importing with image URLs)
Options.register(
    Option(
        display_name=JSONL.name,
        task_id=ProjectType.IMAGE_CLASSIFICATION,
        file_format=JSONL,
        arg=ArgColumn,
        file=IMAGE_CLASSIFICATION_DIR / "example.jsonl",
    )
)