...
Serves files with correct MIME types from the Vue dist directory.
"""
...
def try_serve_static(request):
    ...
    Returns FileResponse if file exists, None otherwise.
    """
    ...
    # Cache headers for production (files are hashed by Vite)
...
def serve_vue_asset(request, path, subdir=''):
    ...
    Args:
        ...
        subdir: Subdirectory within Vue dist (e.g., '_app', '_fonts')
    """
    vue_dist = get_vue_dist_path()
...
def serve_fonts(request, path):
    """Serve files from /_fonts/"""
    return serve_vue_asset(request, path, subdir='_fonts')
